...
inline D go(D a ,D b){return max(a*t1+b*t2-(a*t1*k/100) , b*t1+a*t2-(b*t1*k)/100);}
...
// 	a.reserve(MAXN);
	...
	for(ll i = 0 ;i < n ;i++){
		cin >> ai >> bi;
		a.PB(MP(go<double>(ai ,bi) ,i+1));
	}
	sort(ALL(a) ,cmp);
	for(pair<double ,ll> & i :a) printf("%d %.2lf\n", i.S, (float)i.F);
								//cout << setprecision(9) << i.S << " " << i.F << "\n";
}
