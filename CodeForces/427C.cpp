...
vector<vector<ll>> g(MAXN ,vector<ll>(0)), gr(MAXN ,vector<ll>(0));
vector<bool> used(MAXN ,0);
vector<ll> order, component ,cost(MAXN);
ll n ,m ,sccCount = 0;
...
int32_t main() {
	...
	mod_int ans = 1 ; ll sm = 0;
	
	auto get = [&](const vector<ll> & C) -> pair<ll ,ll> {
	    ll cnt = 0 ,mn = 1e15;
	    for(const ll& u : C){
	        mn = min(mn ,cost[u]);
	    }
	    for(const ll& u : C) cnt += (mn == cost[u]);
	    return MP(mn ,cnt);
	};
	
	for (ll i=0; i<n; ++i) {
		ll v = order[n-1-i]; // for stack based ds
		if (!used[v]) {
			dfs2 (v);
			pair<ll ,ll> ret = get(component);
			ans *= ret.S;
			sm += ret.F;
// 			cout << ret;
